...
CREATE TABLE "order" (
    ...
    CONSTRAINT status_options CHECK (status == "waiting" OR status == "processing" OR status == "shipped" OR status == "delivered"),
    CONSTRAINT employee_assignment CHECK ((status == "waiting" AND employee_id IS NULL) OR (status != "waiting" AND employee_id IS NOT NULL))
);